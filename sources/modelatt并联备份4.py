...
class Model2:
    ...
    def __init__(self, charList, restore=False):
        ...
        # input images
        self.inputImgs = tf.placeholder(tf.float32, shape=(None, Model2.imgSize[0], Model2.imgSize[1]))
    ...
    def trainBatch(self, minibatch):
        ...
        feed = {self.inputImgs: minibatch.imgs, self.gtTexts: sparse,
                self.seqLen: [Model2.maxTextLen] * numBatchElements, self.learningRate: lr, self.keepprob: 0.75,
                self.is_train: True}
    ...
    def inferBatch(self, minibatch):
        ''' recognize the texts '''
        numBatchElements = len(minibatch.imgs)
        feed = {self.inputImgs: minibatch.imgs, self.seqLen: [Model2.maxTextLen] * numBatchElements, self.keepprob: 1.0,
                self.is_train: False}
    ...
    def save(self):
        ''' save model to file '''
        self.ID += 1
        self.saver.save(self.sess, '../model2/crnn-model', global_step=self.ID)
