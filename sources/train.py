...
def train(model, loader):
    ...
    while True:
        ...
        loader.trainSet()
        while loader.next():
            info = loader.getIterator()  # return current minibatch index and number of minibatches
    ...
    print('最高Character Accuracy为：', 1 - min(listCharErrorRate))
    print('最高Word Accuracy为：', max(listAcc))
...
def main():
    loader = DataLoader('../data/test21245v6/train/',
                        '../data/test21245v6/test/' ,
                        Model.batchSize, Model.imgSize)
    # loader = DataLoader('/home/wzw/桌面/vietnamese-handwriting-recognition-master/data/opendata/', Model.batchSize, Model.imgSize)
    # Model.batchSize  Model.imgSize = 64 (128, 32)
    # save charList in data folder
    print(loader.charList)
    strChars = str()
    with open('../data/test2000.txt', 'w', encoding='utf-8') as f:
